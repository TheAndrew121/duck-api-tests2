...
public class DuckPropertiesTest extends DuckClient {
    ...
    public void testGetPropertiesRubber(@Optional @CitrusResource TestCaseRunner runner,
        ...
        duckClientDB.validateDuckInDatabase(runner, duckId, "red", "50.0", "rubber", "quack", "FIXED");
    ...
    private String createWoodDuckWithEvenId(TestCaseRunner runner, TestContext context) {
        for (int attempt = 1; attempt <= 2; attempt++) {
            ...
            String duckIdTemplate = duckClientDB.createDuckInDatabase(runner, "brown", 0.2, "wood", "quack", "FIXED");
    ...
    private String createRubberDuckWithOddId(TestCaseRunner runner, TestContext context) {
        for (int attempt = 1; attempt <= 2; attempt++) {
            String duckIdTemplate = duckClientDB.createDuckInDatabase(runner, "red", 50.0, "rubber", "quack", "FIXED");
            ...
            if (!isEven) {
                System.out.println("Создана утка с нечётным ID: " + actualDuckId + " (попытка: " + attempt + ")");
                return actualDuckId;
            } else {
                System.out.println("Rubber утка создана с чётным ID: " + actualDuckId + " - удаляем и пробуем снова");
                duckClientDB.deleteDuckFromDatabase(runner, actualDuckId);
            }
        }

        throw new RuntimeException("Не удалось создать утку с нечётным ID после 2 попыток");
    }
}